...
{
#ifdef NOTDEF
// you need to check that  malloc() worked
	if(front == NULL) {
		node *new_node = (node*)malloc(sizeof(node));
		new_node->DNSname = malloc(sizeof(DNSname));
	...
	node *new_node;
  	...
  	if ((new_node = (node *)malloc(sizeof(node))) != NULL) {
		new_node->next = front;
       	if ((new_node->DNSname = strdup(DNSname)) == NULL) {
			(void)free(new_node);
			new_node = NULL;
		}
  	}
...
#ifdef NOTDEF		
		/*  you cannot be sure the line ends in a newline....*/
		unsigned long length = strlen(line_buf);
		line_buf[length-1] = '\0';
#endif
		if ((substr = strchr(line_buf, '\n')) != NULL) {
			*substr = '\0';
		}
...
#ifdef NOTDEF
// you forgot to call dnslookup()
			if((temp = htable[hash(line_buf)%tabsz]) == NULL) {
